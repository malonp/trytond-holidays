...
class Calendar(ModelSQL, ModelView):
    ...
    owner = fields.Many2One(
        'res.user', 'Owner', select=True, domain=[('email', '!=', None)], help='The user must have an email'
    )
    read_users = fields.Many2Many('holidays.calendar-read-res.user', 'calendar', 'user', 'Read Users')
    write_users = fields.Many2Many('holidays.calendar-write-res.user', 'calendar', 'user', 'Write Users')
    ...
    def __setup__(cls):
        ...
        t = cls.__table__()
        cls._sql_constraints = [('name_uniq', Unique(t, t.name, t.owner), 'The name of calendar must be unique.')]
    ...
    def calendar2dates(
        self, from_date=datetime.date.today(), to_date=datetime.date.today() + relativedelta.relativedelta(years=1)
    ):
        '''
...
class ReadUser(ModelSQL):
    ...
    calendar = fields.Many2One('holidays.calendar', 'Calendar', ondelete='CASCADE', required=True, select=True)
    user = fields.Many2One('res.user', 'User', ondelete='CASCADE', required=True, select=True)
...
class Event(ModelSQL, ModelView):
    ...
    _rec_name = 'uuid'
    uuid = fields.Char('UUID', required=True, help='Universally Unique Identifier', select=True)
    calendar = fields.Many2One('holidays.calendar', 'Calendar', required=True, select=True, ondelete="CASCADE")
    ...
    status = fields.Selection(
        [('tentative', 'Tentative'), ('confirmed', 'Confirmed'), ('cancelled', 'Cancelled')], 'Status', required=True
    )
    rdates = fields.One2Many('holidays.event.rdate', 'event', 'Recurrence Dates')
    rrules = fields.One2Many('holidays.event.rrule', 'event', 'Recurrence Rules')
    exdates = fields.One2Many('holidays.event.exdate', 'event', 'Exception Dates')
    exrules = fields.One2Many('holidays.event.exrule', 'event', 'Exception Rules')
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints = [
            ('uuid_recurrence_uniq', Unique(t, t.uuid, t.calendar), 'UUID must be unique in a calendar.')
        ]
    ...
    def write(cls, *args):
        ...
        for sub_ids in grouped_slice(events, transaction.database.IN_MAX):
            ...
            cursor.execute(*table.update(columns=[table.sequence], values=[table.sequence + 1], where=red_sql))
    ...
    def copy(cls, events, default=None):
        ...
        for event in events:
            ...
            new_events.extend(super(Event, cls).copy([event], default=current_default))
        return new_events
    ...
    def event2dates(
        ...
        if to_date or self.dtend:
            rs.exrule(rrule.rrule(rrule.DAILY, dtstart=min(filter(None, [to_date, self.dtend]))))
        if self.rrules:
        ...
        if self.rdates:
            rs.rrule(
                [
                    datetime.datetime(rd.date.year, rd.date.month, rd.date.day)
                    for rd in self.rdates
                    if rd.date >= from_date
                ]
            )
        if self.exdates:
            rs.exrule([datetime.datetime(ex.date.year, ex.date.month, ex.date.day) for ex in self.exdates])
        if not (self.rdates or self.rrules or self.exdates or self.exrules):
            rs.rrule(rrule.rrule(rrule.DAILY, dtstart=from_date, count=1 if not self.dtend else None, until=self.dtend))
...
class EventRDate(ModelSQL, ModelView):
    ...
    event = fields.Many2One('holidays.event', 'Event', ondelete='CASCADE', select=True, required=True)
...
class RRuleMixin(Model):
    ...
    freq_num = fields.Function(fields.Integer('Frequency Value'), getter='get_freq_num')
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints += [
            (
                ...
                Check(t, (t.until == Null) | (t.count == Null) | (t.count == 0)),
                'Only one of "until" and "count" can be set.',
            )
        ]
        cls._error_messages.update(
            {
                'invalid_bysetpos': ('Invalid "By Position" in recurrence rule "%s"'),
                'invalid_bymonth': ('Invalid "By Month" in recurrence rule "%s"'),
                'invalid_bymonthday': ('Invalid "By Month Day" in recurrence ' 'rule "%s"'),
                'invalid_byyearday': ('Invalid "By Year Day" in recurrence ' 'rule "%s"'),
                'invalid_byweekno': ('Invalid "By Week Number" in recurrence ' 'rule "%s"'),
                'invalid_byday': 'Invalid "By Day" in recurrence rule "%s"',
                'invalid_byeaster': ('Invalid "By Easter" in recurrence ' 'rule "%s"'),
            }
        )
        ...
        if self.bymonthday:
            for monthdaynum in self.bymonthday.split(','):
                ...
                if not (abs(monthdaynum) >= 1 and abs(monthdaynum) <= 31):
                    self.raise_user_error('invalid_bymonthday', (self.rec_name,))
        ...
        if self.byeaster:
            for easternum in self.byeaster.split(','):
                ...
                if not (abs(easternum) <= 366):
                    self.raise_user_error('invalid_byeaster', (self.rec_name,))
